...
class DetrDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # image, box = self.get_fixed_data(self.annotation_lines[index], self.input_shape[0:2])  # 图像预处理（保持不变）
    ...
    def get_fixed_data(self, annotation_line, input_shape):
        """固定处理函数，无任何随机增强"""
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.05, hue=.05, sat=0.05, val=0.05, random=True):
        line = annotation_line.split()
        # ------------------------------#
        #   读取图像并转换成RGB图像
        # ------------------------------#
